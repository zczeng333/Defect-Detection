...
def noising():
    ...
    skimage.io.imsave(path + 'image_gaussian.jpg', image_gaussian)
    skimage.io.imsave(path + 'image_salt.jpg', image_salt)
    skimage.io.imsave(path + 'image_pepper.jpg', image_pepper)


class denoising(object):  # 定义领域平均化去噪类
    def __init__(self):
        self.filter = np.ones((5, 5))  # 定义卷积核
        ...
        for k in range(self.dic.shape[2]):
            for j in range(self.zp, self.dic.shape[1] + self.zp):
                for i in range(self.zp, self.dic.shape[0] + self.zp):
                    ...
                    self.output[i - self.zp, j - self.zp, k] = np.sum(temp * self.filter) / (self.size * 255)
        return self.output
...
if __name__ == '__main__':
    ...
    output = func.conv(path + 'image_gaussian.jpg')
    skimage.io.imsave(path + 'imagenew_gaussian(5).jpg', output)  # 高斯噪声图像处理
    output = func.conv(path + 'image_salt.jpg')
    skimage.io.imsave(path + 'imagenew_salt(5).jpg', output)  # 盐噪声图像处理
    output = func.conv(path + 'image_pepper.jpg')
    skimage.io.imsave(path + 'imagenew_pepper(5).jpg', output)  # 椒噪声图像处理
